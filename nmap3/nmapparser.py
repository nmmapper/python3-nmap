...
def convert_xml_elements(xml_obj):
    elements = {}
    elem_counter = 0
...
class NmapCommandParser(object):
    ...
    def filter_top_ports(self, xmlroot):
        ...
        try:
            ...
            for hosts in scanned_host:
                ...
                port_result_dict[address]["osmatch"] = parse_os(hosts)
                port_result_dict[address]["ports"] = self.parse_ports(hosts)
                port_result_dict[address]["hostname"] = parse_hostnames(hosts)
                port_result_dict[address]["macaddress"] = parse_mac_address(hosts)
                port_result_dict[address]["state"] = get_hostname_state(hosts)

            port_result_dict["stats"] = stats
            port_result_dict["runtime"] = parse_runtime(xmlroot)
    ...
    def os_identifier_parser(self, xmlroot):
        ...
        try:
            ...
            for host in hosts:
                ...
                os_dict[address]["osmatch"] = parse_os(host)
                os_dict[address]["ports"] = self.parse_ports(host)
                os_dict[address]["hostname"] = parse_hostnames(host)
                os_dict[address]["macaddress"] = parse_mac_address(host)

            os_dict["runtime"] = parse_runtime(xmlroot)
            os_dict["stats"] = stats
            return os_dict
    ...
    def parse_scripts(self, scripts_xml):
        ...
        for script_xml in scripts_xml:
            ...
            data = convert_xml_elements(script_xml)
    ...
    def convert_xml_tables(self, xml_tables):
        ...
        for xml_table in xml_tables:
            ...
            child_data = convert_xml_elements(xml_table)
            if key is None:
                if {} != child_data:
                    a = data.get('children', [])
